...
;;          (keys (hash-table-keys entries)))
;;     (dolist (key keys)
;;       ;; Check org-roam database directly for notes with this citation key
;;       (let ((existing-nodes (org-roam-db-query
;;                              [:select [id]
;;                                       :from nodes
;;                                       :where (like title $s1)]
;;                              (concat "%" key "%"))))
        
;;         (when (null existing-nodes)
;;           ;; Create note
;;           (message "Creating note for: %s" key)
;;           (citar-create-note (list key))
...
;;                               (concat "%" key "%")))
;;                       (node-id (caar nodes))
;;                       (node-file (cadar nodes)))
            
;;             ;; Visit the note file
;;             (with-current-buffer (find-file-noselect node-file)
;;               (goto-char (point-max))
;;               ;; Insert "Research" tag using your custom function
;;               (let ((org-roam-node-read 
;;                      (lambda (&optional _ _) 
;;                        (org-roam-node-from-title-or-alias "Research"))))
;;                 (org-roam-tag-node-insert))
;;               ;; Insert "unindexed" tag using your custom function
;;               (let ((org-roam-node-read 
;;                      (lambda (&optional _ _) 
;;                        (org-roam-node-from-title-or-alias "unindexed"))))
;;                 (org-roam-tag-node-insert))
;;               ;; Save the buffer
;;               (save-buffer)))))))) 
#+end_src
