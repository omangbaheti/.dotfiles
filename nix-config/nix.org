...
- [[#individual-configurations][Individual Configurations]]
  ...
  - [[#wsl-config-files][WSL Config Files]]
    - [[#configurationnix-4][Configuration.nix]]
    - [[#homenix-4][Home.nix]]
  - [[#annie---pixel][Annie - Pixel]]
    - [[#configurationnix-5][Configuration.nix]]
    - [[#homenix-5][Home.nix]]
...
** Vultr - VPS
*** Configuration.nix
#+begin_src nix :noweb yes :tangle ~/.dotfiles/nix-config/vultr/configuration.nix
{ config, pkgs, lib, stable, machine, inputs, ... }:
let
  hostname = machine.host;
  username = machine.username;
...
*** Home.nix
#+begin_src nix :noweb yes :tangle ~/.dotfiles/nix-config/vultr/home.nix
{ config, pkgs, stable, machine, inputs, ... }:
{
  home.stateVersion = "26.05";
...
** Annie - Pixel 
*** Configuration.nix
#+begin_src nix :noweb yes :tangle ./annie/configuration.nix
{config, lib, pkgs, stable, machine, ... }:
let
  userName = machine.username;
  allowUnfree = machine.allowUnfree;
...
*** Home.nix
#+begin_src nix :noweb yes :tangle ./annie/home.nix
