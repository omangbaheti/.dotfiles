...
#+begin_src jupyter-python :tangle temp.py :session py :async yes :pandoc t
def create_batch_file(pdf_paths, output_file):
...
# Create batch file from your PDFs
pdf_files = ["/home/nixos/Notes/Files/FingerTextExploringOptimizing_Lee_2021.pdf", "/home/nixos/Notes/Files/AllOneNeeds_Lee_2021.pdf"]
create_batch_file(pdf_files, "batch_ocr.jsonl")
#+end_src 
...
batch_data = client.files.upload(
    file={
        "file_name": "/home/nixos/Notes/batches/batch_1.jsonl",
        "content": open("/home/nixos/Notes/batches/batch_1.jsonl", "rb")
    },
    purpose="batch"
)
...
while True:
    ...
    print(f"Successful: {retrieved_job.succeeded_requests}")
    print(f"Failed: {retrieved_job.failed_requests}")
...
:RESULTS:
: Hello
|    | batch_file     | job_id                               | status  | total | succeeded | failed |
|----+----------------+--------------------------------------+---------+-------+-----------+--------|
...
#+begin_src jupyter-python :tangle temp.py :session py :async yes :pandoc t :results none
input_path = "/home/nixos/Notes/batch_results.jsonl"
entries_path = Path("/home/nixos/Notes/citar-entries.jsonl")
out_root = Path("/home/nixos/Notes/markdown_docs")
